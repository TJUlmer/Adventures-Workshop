-- Gallery card previews now use WebP at their original pixel dimensions.
-- Continue accepting PNG so a browser without WebP encoding can publish and
-- existing admin refreshes do not fail during a staggered client rollout.
...
begin
  ...
  loop
    ...
    if asset_name !~ ('^' || auth.uid()::text || '/' || target::text ||
        '/card-preview-[0-9a-f]{8}-[0-9a-f]{8}[.](png|webp)$')
       or not exists (
         select 1
