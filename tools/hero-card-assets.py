...
# The four character-card layouts, by the suffix their derived pieces take.
CHARACTER_FRAMES = {
    ...
    "_multi_lowhealth": "Hero_Character_Card_Template_frame_multisidekick_lowhealth.png",
}
...
# Suffixes for every character-card ink file — the four this script's own
...
# in `renderer/assets.ts`) but shares this exact row with the other four,
# confirmed pixel-for-pixel below rather than assumed.
CHARACTER_INK_SUFFIXES = ["", "_sidekick", "_multi", "_multi_lowhealth", "_multihealth"]
...
def split_move_ink() -> None:
    ...
    All five ink files carry the identical shape at the identical position —
    ...
    **This stage erases from the files it reads**, so it cannot run twice: the
    ...
    of all five ink files, which does not exist. Until it does, an already-run
...
# pixel-identical in all five ink files (asserted below, not assumed), so each
...
SIDEKICK_LABEL_FILES = {
    ...
    "_multi_lowhealth": "hero_character_label_ink_sidekick.png",
...
def split_label_ink() -> None:
    ...
    **This stage erases from the files it reads**, exactly as `split_move_ink`
    ...
    the five ink files to read instead. So an already-run band is *skipped*
    ...
    for suffix in CHARACTER_INK_SUFFIXES:
        ...
        for band in CHARACTER_LABEL_BANDS:
            ...
            # all five files, and `_sidekick`/`_multi`/`_multi_lowhealth` in the three that share.
            if name in written:
...
def main() -> None:
    ...
    def soften_badge_edge(image: Image.Image) -> Image.Image:
        """Keep one sub-pixel coverage ramp around an otherwise hard badge.

        A frame mask must meet each band with a hard edge: blending its pink
        into an arbitrary author-coloured band makes a visible halo. The badge
        sits inside one band instead, and its small diagonal shield edge is far
        more conspicuous when it is reduced to binary coverage. A narrow blur
        restores the antialiasing the flat source image cannot carry in alpha,
        without turning the badge into a visibly soft shape.
        """
        return mask_png(np.array(image.filter(ImageFilter.GaussianBlur(radius=1.25)))[:, :, 3])

    def soften_badge_arcs(image: Image.Image, badge_shapes: np.ndarray) -> Image.Image:
        """Restore coverage only where a health badge's curved frame crosses it.

        The source's pink arc is not transparent art: it is a solid-colour
        drawing, so colour matching turns its antialiased edge into a binary
        mask. Softening the whole border would bring back the halos `harden`
        prevents at every band join. Each blue shield in the source instead
        locates the small arc surrounding it, including a sidekick's lower
        badge when the supplied frame has one.
        """
        alpha = np.array(image)[:, :, 3]
        blurred = np.array(image.filter(ImageFilter.GaussianBlur(radius=1.25)))[:, :, 3]
        labels, count = ndimage.label(badge_shapes)
        scale_x = BLEED[0] / badge_shapes.shape[1]
        scale_y = BLEED[1] / badge_shapes.shape[0]
        ...
        for component in range(1, count + 1):
            ...
            # The arc crosses the shield's top, then extends a little to both
            # sides. It never reaches the band dividers below it.
            x0 = max(0, int(columns.min() * scale_x) - 36)
            x1 = min(alpha.shape[1], int((columns.max() + 1) * scale_x) + 36)
            y0 = max(0, int(rows.min() * scale_y) - 64)
            y1 = min(alpha.shape[0], int((rows.min() + 1) * scale_y) + 92)
            alpha[y0:y1, x0:x1] = blurred[y0:y1, x0:x1]

        return mask_png(alpha)
    ...
    for name, source in CHARACTER_FRAMES.items():
        ...
        badge_shapes = (
            np.abs(frame_art[:, :, :3] - np.array(BADGE_BLUE)).sum(2) < 60
        ) & opaque
        badge = badge_shapes.copy()
        ...
        border_mask = harden(to_bleed(mask_png(border)))
        soften_badge_arcs(border_mask, badge_shapes).save(
            TEMPLATES / f"hero_character_border{name}.png"
        )
        soften_badge_edge(to_bleed(mask_png(np.where(badge, frame_art[:, :, 3], 0)))).save(
            TEMPLATES / f"hero_character_badge{name}.png"
        )
        soften_badge_edge(to_bleed(mask_png(np.where(accent, frame_art[:, :, 3], 0)))).save(
