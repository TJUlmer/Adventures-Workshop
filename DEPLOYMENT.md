...
Collection creation is available to every signed-in permanent account when sharing is
configured. Home hides **New collection** for signed-out and temporary accounts. Supabase
RLS authorises the underlying writes; the Home button is not a security boundary. The
former `VITE_COLLECTIONS_INTERNAL_USER_IDS` allowlist is no longer read by the app and
may be removed from Vercel's environment settings after this release.
