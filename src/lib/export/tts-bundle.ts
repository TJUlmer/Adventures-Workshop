  ...
  await withCardStage(async (photograph) => {
    for (const [memberIndex, member] of members.entries()) {
      ...
      for (const plan of plans) {
        ...
        const sheets: TtsSheet[] = [];
        for (const [page, sheet] of rendered.entries()) {
          /* Not namespaced per creator, because `plan.id` already names the
             pile after the character it belongs to, and two creators' piles of
             the same name are told apart by their own content hash. An author
...
Each was drawn from its own author's published set, under that author's own
...
-----
  Card quantities are expanded: a card with ×3 is three cards in the pile.
  A pile over 70 cards is split across several sheets and is still one pile.
  Identical artwork is stored once however many decks use it, including
  across creators, so the folder is no larger than it has to be.
