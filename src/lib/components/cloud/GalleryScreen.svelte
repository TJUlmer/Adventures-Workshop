<script lang="ts">
  ...
  async function load(query: Query, append = false): Promise<void> {
    ...
    try {
      ...
      /* Collections answer first and alone. `list_public_collections` takes a
         limit and nothing else — no search, sort, paging, and nothing to
         favourite — so the favourites gate below would only ever be a
         question about sets and characters. */
  ...
  {:else}
    <div class="controls">
      ...
      <!--
        ...
        than shown doing nothing: `list_public_collections` takes a limit and
        nothing else, and a collection is not a thing that can be favourited.
        A control that is present and inert reads as broken — the same rule
        `StylePanel` follows for surfaces a template does not draw.
      -->
      {#if mode !== 'collections'}
        <button
          type="button"
